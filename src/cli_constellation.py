...
def run_single_mission_optimization(
    config: Dict[str, Any], output_file: Optional[str] = None
) -> Dict[str, Any]:
...
def run_constellation_optimization(
    config: Dict[str, Any],
    num_missions: int,
...
def print_results_summary(results: Dict[str, Any], num_missions: int = 1):
    ...
    if best_solutions:
        ...
        for i, sol in enumerate(best_solutions):
            ...
            print(
                f"  {i+1}. ΔV: {delta_v:.0f} m/s, Time: {time:.1f} days, Cost: ${cost:.1f}M"
            )
    ...
    if num_missions > 1:
        ...
        if const_metrics:
            ...
            if coverage_stats:
                print(
                    f"  Coverage: {coverage_stats.get('mean', 0):.2f} ± {coverage_stats.get('std', 0):.2f}"
                )

            redundancy_stats = const_metrics.get("redundancy_stats", {})
            if redundancy_stats:
                print(
                    f"  Redundancy: {redundancy_stats.get('mean', 0):.2f} ± {redundancy_stats.get('std', 0):.2f}"
                )
...
Examples:
  ...
  python cli_constellation.py config/lunar_mission.yaml --multi 8 \\
    ...
    parser.add_argument(
        "--constellation-weights",
        help='Constellation objective weights (e.g., "coverage=2.0,redundancy=1.0")',
    )

    # Output options
    parser.add_argument("--output", "-o", help="Output file for results (JSON or YAML)")
    ...
    try:
        ...
        if args.constellation_weights:
            constellation_weights = parse_constellation_weights(
                args.constellation_weights
            )
        ...
        if args.multi and args.multi > 1:
            ...
            results = run_single_mission_optimization(
                config=config, output_file=args.output
            )
            num_missions = 1
