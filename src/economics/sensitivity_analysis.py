...
class EconomicSensitivityAnalyzer:
    ...
    def one_way_sensitivity(self,
        ...
        logger.info(f"One-way sensitivity analysis complete. Most sensitive: {ranked_variables[0][0]}")
    ...
    def tornado_diagram_data(self,
        ...
        for variable, (min_val, max_val) in variable_ranges.items():
            ...
            low_npv = low_result.get("npv", 0) if isinstance(low_result, dict) else low_result

            # Calculate high case
            high_params = base_parameters.copy()
            high_params[variable] = max_val
            high_result = self.base_model_function(high_params)
            high_npv = high_result.get("npv", 0) if isinstance(high_result, dict) else high_result
    ...
    def scenario_analysis(self,
        ...
        for scenario_name, scenario_params in scenarios.items():
            ...
            scenario_npv = scenario_result.get("npv", 0) if isinstance(scenario_result, dict) else scenario_result
    ...
    def monte_carlo_simulation(self,
        ...
        percentiles = np.percentile(valid_results_array, [p * 100 for p in confidence_levels])
