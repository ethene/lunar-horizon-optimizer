...
class OptimizationVisualizer:
    ...
    def __init__(self, config: Optional[ParetoPlotConfig] = None):
        """
    ...
    def create_optimization_convergence_plot(
        ...
        for obj_idx, (values, name, color) in enumerate(zip(best_objectives, objective_names, colors)):
            fig.add_trace(
                go.Scatter(
                    x=generations,
                    y=values,
    ...
    def create_solution_comparison_plot(
        ...
        ranking_data = []
        for i, (sol, label) in enumerate(zip(solutions, solution_labels)):
            ranking_data.append([
                label,
    ...
    def create_preference_analysis_plot(
        ...
        colors = px.colors.qualitative.Set1[:n_objectives]
        for obj_idx, (obj_name, weight, color) in enumerate(zip(objective_names, preference_weights, colors)):
            weighted_values = []
