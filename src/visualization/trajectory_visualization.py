...
class TrajectoryVisualizer:
    ...
    def __init__(self, config: Optional[TrajectoryPlotConfig] = None):
        """
    ...
    def create_3d_trajectory_plot(
        self,
        trajectories: Union[Dict[str, Any], List[Dict[str, Any]]],
    ...
    def create_orbital_elements_plot(
        ...
        if show_evolution:
            ...
            for idx, (name, unit) in enumerate(zip(element_names, element_units)):
                row = (idx // 2) + 1
                col = (idx % 2) + 1
    ...
    def _add_trajectory_path(
        ...
        if 'times' in trajectory:
            ...
            hover_text = [f"Time: {t:.2f}<br>Position: ({x:.0f}, {y:.0f}, {z:.0f}) km" 
                         for t, x, y, z in zip(times, x_km, y_km, z_km)]
        else:
            hover_text = [f"Position: ({x:.0f}, {y:.0f}, {z:.0f}) km" 
                         for x, y, z in zip(x_km, y_km, z_km)]
...
def create_quick_trajectory_plot(
    ...
    try:
        ...
        fig = go.Figure()
        fig.add_annotation(
            text=f"Trajectory generation failed: {str(e)}",
            xref="paper", yref="paper",
            x=0.5, y=0.5, showarrow=False,
