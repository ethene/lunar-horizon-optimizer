...
if RAY_AVAILABLE:
    ...
    class FitnessWorker:
        ...
        def __init__(
            ...
            logger.info(
                f"FitnessWorker {worker_id} initialized with pre-loaded resources"
            )
        ...
        def _initialize_resources(self):
            ...
            try:
                ...
                logger.debug(
                    f"Worker {self.worker_id}: Resources pre-loaded successfully"
                )

            except Exception as e:
                logger.warning(
                    f"Worker {self.worker_id}: Resource initialization failed: {e}"
                )
        ...
        def evaluate_batch(
            self, population_chunk: List[List[float]]
        ) -> List[List[float]]:
            ...
            logger.debug(
                f"Worker {self.worker_id}: Evaluated {len(population_chunk)} individuals in {batch_time:.3f}s"
            )
        ...
        def _evaluate_single(self, x: List[float]) -> List[float]:
            ...
            if (
                earth_alt < self.bounds["min_earth_alt"]
                or earth_alt > self.bounds["max_earth_alt"]
                or moon_alt < self.bounds["min_moon_alt"]
                or moon_alt > self.bounds["max_moon_alt"]
                or transfer_time < self.bounds["min_transfer_time"]
                or transfer_time > self.bounds["max_transfer_time"]
            ):
            ...
            try:
                ...
                logger.debug(
                    f"Worker {self.worker_id}: Fitness evaluation failed for {x}: {e}"
                )
        ...
        def get_stats(self) -> dict:
            ...
            hit_rate = (
                self._cache_hits / total_evaluations if total_evaluations > 0 else 0
            )
            avg_time = (
                self.total_time / self.evaluation_count
...
class RayParallelOptimizer(GlobalOptimizer):
    ...
    def __init__(
        ...
        self.chunk_size = chunk_size or max(
            1, population_size // (self.num_workers * 2)
        )
        ...
        if not self.use_ray:
            ...
            logger.info(
                f"RayParallelOptimizer: {self.num_workers} workers, chunk_size={self.chunk_size}"
            )
    ...
    def _shutdown_ray_workers(self):
        ...
        try:
            ...
            total_cache_misses = sum(
                stats["cache_misses"] for stats in self.worker_stats
            )

            if total_evaluations > 0:
                avg_hit_rate = total_cache_hits / (
                    total_cache_hits + total_cache_misses
                )
                logger.info(
                    f"Ray workers completed: {total_evaluations} evaluations, "
    ...
    def _optimize_with_ray(self, verbose: bool = True) -> dict[str, Any]:
        ...
        """
        logger.info(
            f"Starting Ray-parallel NSGA-II optimization: {self.num_generations} generations"
        )
        ...
        for generation in range(self.num_generations):
            ...
            for _i, (x, f) in enumerate(zip(population_x, population_f, strict=False)):
        ...
        if verbose:
            logger.info(
                f"Ray-parallel optimization completed: {len(final_pareto)} Pareto solutions found"
            )

        return results

    def _evaluate_population_parallel(
        self, population_x: np.ndarray
    ) -> List[List[float]]:
        ...
        logger.debug(
            f"Parallel evaluation: {len(population_x)} individuals in {eval_time:.3f}s "
...
def create_ray_optimizer(
    ...
    ray_config: Optional[dict] = None,
) -> Union[RayParallelOptimizer, GlobalOptimizer]:
    ...
    if RAY_AVAILABLE:
        optimizer = RayParallelOptimizer(
            problem=problem, ray_config=ray_config, **optimizer_config
        )
        logger.info("Created RayParallelOptimizer")
    else:
        optimizer = GlobalOptimizer(problem=problem, **optimizer_config)
