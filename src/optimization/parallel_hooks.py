...
class PopulationEvaluator(Protocol):
    ...
    def evaluate_population(self, 
                          ...
                          fitness_func: Callable[[List[float]], List[float]]) -> List[List[float]]:
...
class RayEvaluator:
    ...
    def __init__(self, num_workers: Optional[int] = None, chunk_size: Optional[int] = None):
...
def add_parallel_evaluation_to_optimizer(optimizer, evaluator: PopulationEvaluator):
    ...
    logger.info(f"Added parallel evaluation to optimizer using {type(evaluator).__name__}")


def get_recommended_evaluator(problem_config: Optional[dict] = None) -> PopulationEvaluator:
