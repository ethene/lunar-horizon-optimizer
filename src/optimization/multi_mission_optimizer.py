...
class MultiMissionOptimizer(GlobalOptimizer):
    ...
    def __init__(
        ...
        if problem is None:
            problem = create_backward_compatible_problem(
                enable_multi=multi_mission_mode, num_missions=num_missions
            )
        ...
        for sol in pareto_solutions:
            ...
            if len(obj_list) >= 5:  # Multi-mission problem
                weighted_score = (
                    ...
                    + weights.get("coverage", 1.0)
                    * obj_list[3]
                    / 1e3  # Normalize coverage
                    + weights.get("redundancy", 0.5)
                    * obj_list[4]
                    / 1e3  # Normalize redundancy
                )
            ...
            enhanced_sol["constellation_analysis"] = (
                self._analyze_constellation_solution(sol)
            )
    ...
    def _analyze_constellation_solution(
        ...
        if len(params) >= 4 * self.num_missions + 2:
            ...
            try:
                genome = MultiMissionGenome.from_decision_vector(
                    params, self.num_missions
                )
...
def optimize_constellation(
    ...
    problem = MultiMissionProblem(
        num_missions=num_missions, cost_factors=cost_factors, **problem_params
    )
...
def migrate_single_to_multi(
    single_config: Dict[str, Any], num_missions: int = 3
) -> Dict[str, Any]:
    ...
    logger.info(
        f"Migrated single-mission config to {num_missions}-mission constellation"
    )
