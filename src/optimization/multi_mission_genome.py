...
class MultiMissionGenome:
    ...
    plane_raan: List[float] = field(default_factory=list)  # deg, orbital plane orientation
    ...
    def __post_init__(self):
        ...
        if not self.plane_raan:
            self.plane_raan = [i * (360.0 / self.num_missions) for i in range(self.num_missions)]
        ...
        if len(self.epochs) != expected_len:
            raise ValueError(f"epochs length {len(self.epochs)} != num_missions {expected_len}")
        if len(self.parking_altitudes) != expected_len:
            raise ValueError(f"parking_altitudes length {len(self.parking_altitudes)} != num_missions {expected_len}")
        if len(self.plane_raan) != expected_len:
            raise ValueError(f"plane_raan length {len(self.plane_raan)} != num_missions {expected_len}")
        if len(self.payload_masses) != expected_len:
            raise ValueError(f"payload_masses length {len(self.payload_masses)} != num_missions {expected_len}")
    
    @classmethod
    def from_decision_vector(cls, x: List[float], num_missions: int) -> 'MultiMissionGenome':
        ...
        if len(x) != expected_length:
            raise ValueError(f"Decision vector length {len(x)} != expected {expected_length}")
    ...
    def get_mission_parameters(self, mission_idx: int) -> dict[str, float]:
        ...
        if not 0 <= mission_idx < self.num_missions:
            raise ValueError(f"Mission index {mission_idx} out of range [0, {self.num_missions})")
    ...
    def validate_constellation_geometry(self) -> bool:
        ...
        if self.num_missions > 1:
            ...
            min_separation = min(
                raan_values[i+1] - raan_values[i] 
                for i in range(len(raan_values)-1)
            )
...
class MultiMissionProblem:
        ...
        try:
            ...
            for i in range(self.num_missions):
                ...
                try:
                    ...
                    total_delta_v += dv
                    total_time += mission_params['transfer_time'] * 86400  # Convert to seconds
            ...
            if self.constellation_mode:
                ...
                redundancy_metric = self._calculate_redundancy_metric(genome, mission_results)
    ...
    def _validate_bounds(self, genome: MultiMissionGenome) -> bool:
        ...
        # Check epoch bounds
        if not all(self.min_epoch <= epoch <= self.max_epoch for epoch in genome.epochs):
            return False
            
        # Check altitude bounds  
        if not all(self.min_earth_alt <= alt <= self.max_earth_alt 
                  for alt in genome.parking_altitudes):
            return False
            
        # Check payload bounds
        if not all(self.min_payload <= mass <= self.max_payload 
        ...
        if not (self.min_moon_alt <= genome.lunar_altitude <= self.max_moon_alt):
            return False
        if not (self.min_transfer_time <= genome.transfer_time <= self.max_transfer_time):
            return False
    ...
    def _calculate_redundancy_metric(self, genome: MultiMissionGenome, 
                                   mission_results: List[dict]) -> float:
        ...
        temporal_diversity = max(0.0, 0.1 - epoch_variance)  # Penalize too much clustering
        
        # Redundancy score (to minimize) 
        redundancy_score = (performance_diversity + temporal_diversity) * self.redundancy_weight
    ...
    **kwargs
    ...
    if enable_multi and num_missions > 1:
        ...
        single_kwargs = {k: v for k, v in kwargs.items() 
                        if k in ['cost_factors', 'min_earth_alt', 'max_earth_alt', 
                               'min_moon_alt', 'max_moon_alt', 'min_transfer_time', 
                               'max_transfer_time', 'reference_epoch']}
