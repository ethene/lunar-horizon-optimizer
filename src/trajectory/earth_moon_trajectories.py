...
class LambertSolver:
    ...
    def solve_lambert(self,
        ...
        try:
            ...
            raise ValueError(f"Lambert problem solution failed: {str(e)}")
...
class OptimalTimingCalculator:
    ...
    def analyze_timing_sensitivity(self,
        ...
        valid_points = [(t, dv) for t, dv in zip(time_variations, sensitivity_data['deltav_variations'])
                       if dv != float('inf')]
        
        if len(valid_points) > 1:
            times, dv_changes = zip(*valid_points)
