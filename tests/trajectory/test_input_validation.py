...
class TestLunarTransferValidation:
    ...
    def setup_method(self):
        ...
        self.departure_time = datetime(2024, 1, 1, 12, 0, 0, tzinfo=UTC)
        ...
        for altitude, error_msg in test_cases:
            with pytest.raises(ValueError, match=error_msg):
                generate_lunar_transfer(
                    self.departure_time, 3.5, altitude, self.nominal_moon_altitude
                )
    ...
    def test_delta_v_constraints(self):
        ...
        with pytest.raises(
            ValueError, match="Maximum delta-v must be greater than minimum"
        ):
            generate_lunar_transfer(
                self.departure_time,
                3.5,
    ...
    def test_max_revolutions_validation(self):
        ...
        # Test negative value
        with pytest.raises(
            ValueError, match="Maximum revolutions must be non-negative"
        ):
            generate_lunar_transfer(
                self.departure_time,
                3.5,
        ...
        with pytest.raises(
            ValueError,
            match=f"Maximum revolutions must be less than {TD.MAX_REVOLUTIONS}",
        ):
            generate_lunar_transfer(
                self.departure_time,
                3.5,
...
def test_invalid_time_of_flight():
    ...
    departure_time = datetime(2024, 1, 1, tzinfo=UTC)
    nominal_altitude = TD.DEFAULT_EARTH_ORBIT
    nominal_moon_altitude = 100.0

    # Test negative time of flight
    with pytest.raises(
        ValueError, match=f"Transfer time must be between {TD.MIN_TRANSFER_TIME}"
    ):
        generate_lunar_transfer(
            departure_time, -1.0, nominal_altitude, nominal_moon_altitude
        )
    ...
    with pytest.raises(
        ...
        generate_lunar_transfer(
            departure_time, 0.0, nominal_altitude, nominal_moon_altitude
        )
    ...
    with pytest.raises(
        ...
        generate_lunar_transfer(
            departure_time, 8.0, nominal_altitude, nominal_moon_altitude
        )
