...
class TestHohmannTransfer:
    ...
    def test_transfer_components(self):
        ...
        logger.debug(
            f"Circular velocities - initial: {v1_circ:.2f} km/s, final: {v2_circ:.2f} km/s"
        )
    ...
    def test_transfer_estimation(self):
        ...
        assert 2.42 <= dv1 <= 2.44, f"First burn {dv1} km/s outside expected range"
        assert 1.46 <= dv2 <= 1.48, f"Second burn {dv2} km/s outside expected range"
        assert 0.21 <= tof <= 0.23, f"Transfer time {tof} days outside expected range"

        logger.debug(
            f"Transfer parameters - ΔV1: {dv1:.2f} km/s, ΔV2: {dv2:.2f} km/s, TOF: {tof:.2f} days"
        )
    ...
    def test_transfer_vectors(self):
        ...
        # Verify magnitudes
        assert (
            abs(np.linalg.norm(r1_vec) - self.r1) < 0.1
        ), "Initial position vector magnitude incorrect"
        assert (
            abs(np.linalg.norm(r2_vec) - self.r2) < 0.1
        ), "Final position vector magnitude incorrect"
        assert (
            abs(np.linalg.norm(v1_vec) - v1_transfer) < 0.1
        ), "Initial velocity vector magnitude incorrect"
        assert (
            abs(np.linalg.norm(v2_vec) - v2_transfer) < 0.1
        ), "Final velocity vector magnitude incorrect"

        logger.debug(
            f"Vector magnitudes verified - r1: {np.linalg.norm(r1_vec):.1f} km, "
    ...
    def test_circular_velocities(self):
        ...
        logger.debug(
            f"Delta-v breakdown - ΔV1: {dv1:.2f} km/s, ΔV2: {dv2:.2f} km/s, "
