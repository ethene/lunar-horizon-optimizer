...
class TestOrbitalMechanicsPhysics:
    ...
    def test_circular_velocity_earth(self):
        ...
        for alt_km in altitudes_km:
            ...
            if alt_km == 200:  # LEO
                assert (
                    7700 <= v_circular <= 7800
                ), f"LEO velocity unrealistic: {v_circular:.0f} m/s"
            elif alt_km == 400:  # ISS altitude
                assert (
                    7600 <= v_circular <= 7700
                ), f"ISS altitude velocity unrealistic: {v_circular:.0f} m/s"
            elif alt_km == 35786:  # GEO
                assert (
                    3000 <= v_circular <= 3100
                ), f"GEO velocity unrealistic: {v_circular:.0f} m/s"

            # General physics validation
            assert v_circular > 0, "Circular velocity must be positive"
            assert (
                v_circular < EARTH_ESCAPE_VELOCITY
            ), "Circular velocity must be less than escape velocity"
    ...
    def test_escape_velocity_validation(self):
        ...
        assert (
            11000 <= v_escape_earth <= 11300
        ), f"Earth escape velocity unrealistic: {v_escape_earth:.0f} m/s"

        # Moon surface escape velocity
        v_escape_moon = math.sqrt(2 * MOON_MU / MOON_RADIUS)
        assert (
            2300 <= v_escape_moon <= 2400
        ), f"Moon escape velocity unrealistic: {v_escape_moon:.0f} m/s"

        # Ratio should be approximately sqrt(M_earth/M_moon * R_moon/R_earth)
        expected_ratio = math.sqrt(
            (EARTH_MASS / MOON_MASS) * (MOON_RADIUS / EARTH_RADIUS)
        )
        actual_ratio = v_escape_earth / v_escape_moon
        assert (
            abs(actual_ratio - expected_ratio) / expected_ratio < 0.1
        ), "Escape velocity ratio inconsistent"
    ...
    def test_orbital_period_validation(self):
        ...
        for semi_major_axis, min_period_min, max_period_min in test_cases:
            ...
            assert (
                min_period_min <= period_minutes <= max_period_min
    ...
    def test_energy_conservation_principle(self):
        ...
        for alt_km in altitudes_km:
            ...
            ), f"Energy conservation violated: {relative_error:.2e}"
    ...
    def test_vis_viva_equation(self):
        ...
        for r_periapsis, r_apoapsis in test_orbits:
            ...
            assert (
                1000 <= v_periapsis <= 15000
            ), f"Periapsis velocity unrealistic: {v_periapsis:.0f} m/s"
            ...
            assert (
                1000 <= v_apoapsis <= 15000
            ), f"Apoapsis velocity unrealistic: {v_apoapsis:.0f} m/s"

            # For elliptical orbits, periapsis velocity > apoapsis velocity
            if r_periapsis < r_apoapsis:
                assert (
                    v_periapsis > v_apoapsis
                ), "Periapsis velocity must be greater than apoapsis velocity"
...
class TestDeltaVValidation:
    ...
    def test_hohmann_transfer_deltav(self):
        ...
        assert (
            3800 <= total_dv <= 4200
        ), f"LEO-GEO Hohmann transfer delta-v unrealistic: {total_dv:.0f} m/s"
        assert (
            2350 <= dv1 <= 2650
        ), f"LEO-GEO departure delta-v unrealistic: {dv1:.0f} m/s"
        assert (
            1350 <= dv2 <= 1650
        ), f"LEO-GEO arrival delta-v unrealistic: {dv2:.0f} m/s"
    ...
    def test_lunar_transfer_deltav_ranges(self):
        ...
        assert (
            3100 <= dv_escape <= 3300
        ), f"Earth escape delta-v unrealistic: {dv_escape:.0f} m/s"

        # Lunar orbit insertion (approximate)
        v_lunar_escape = math.sqrt(
            2 * MOON_MU / (MOON_RADIUS + 100e3)
        )  # 100 km altitude
        ...
        assert (
            LUNAR_CAPTURE_DELTAV_MIN <= dv_lunar_capture <= LUNAR_CAPTURE_DELTAV_MAX
        ), f"Lunar capture delta-v unrealistic: {dv_lunar_capture:.0f} m/s"

        # Total lunar mission delta-v (one way, simplified)
        total_lunar_dv = dv_escape + dv_lunar_capture
        assert (
            3800 <= total_lunar_dv <= 4200
        ), f"Total lunar transfer delta-v unrealistic: {total_lunar_dv:.0f} m/s"
    ...
    def test_interplanetary_deltav_ranges(self):
        ...
        # Realistic ranges
        assert (
            11000 <= v_earth_escape <= 11300
        ), f"Earth escape velocity unrealistic: {v_earth_escape:.0f} m/s"
        assert (
            29000 <= v_earth_orbital <= 30000
        ), f"Earth orbital velocity unrealistic: {v_earth_orbital:.0f} m/s"
...
class TestSpacecraftEngineering:
    ...
    def test_mass_ratio_validation(self):
        ...
        test_cases = [
            ...
            (
                ...
                15.0,
                30.0,
            ),  # Medium Isp, high delta-v (adjusted for realistic mass ratios)
        ]
        ...
        for isp_s, delta_v_ms, min_mass_ratio, max_mass_ratio in test_cases:
            ...
            assert (
                min_mass_ratio <= mass_ratio <= max_mass_ratio
            ...
            if delta_v_ms <= 5000:  # Feasible missions
                assert (
                    payload_fraction >= 0
                ), f"Negative payload fraction: {payload_fraction:.3f}"
                assert (
                    payload_fraction <= MAX_PAYLOAD_FRACTION
                ...
                calculated_total = (
                    structural_fraction + propellant_fraction + payload_fraction
                )
                assert (
                    abs(calculated_total - 1.0) < 1e-10
                ), "Mass fraction conservation violated"
    ...
    def test_thrust_to_weight_ratios(self):
        ...
        for phase, (min_tw, max_tw) in mission_phases.items():
            assert (
                MIN_THRUST_TO_WEIGHT <= min_tw <= MAX_THRUST_TO_WEIGHT
            ), f"{phase} minimum T/W ratio unrealistic: {min_tw:.3f}"
            assert (
                MIN_THRUST_TO_WEIGHT <= max_tw <= MAX_THRUST_TO_WEIGHT
    ...
    def test_spacecraft_mass_components(self):
        ...
        calculated_total = (
            payload_mass + propellant_mass + structural_mass + margin_mass
        )
        ...
        assert (
            structural_fraction >= MIN_STRUCTURAL_FRACTION
        ), "Structural fraction too low"
...
class TestUnitConsistencyValidation:
    ...
    def test_distance_unit_consistency(self):
        ...
        for dist_km in test_distances_km:
            ...
            assert (
                abs(converted_back - dist_km) < 1e-10
    ...
    def test_velocity_unit_consistency(self):
        ...
        for vel_ms in test_velocities_ms:
            ...
            assert (
                abs(converted_back - vel_ms) < 1e-10
    ...
    def test_time_unit_consistency(self):
        ...
        for time_days in test_times_days:
            ...
            assert (
                abs(converted_days_from_sec - time_days) < 1e-10
            ), f"Time conversion error (seconds): {time_days} days != {converted_days_from_sec} days"
            assert (
                abs(converted_days_from_hr - time_days) < 1e-10
    ...
    def test_energy_unit_consistency(self):
        ...
        for alt_km in test_altitudes_km:
            ...
            relative_error = abs(total_specific - specific_energy) / abs(
                specific_energy
            )
            assert (
                relative_error < 1e-10
            ), f"Energy unit consistency error: {relative_error:.2e}"
...
class TestMissionConstraintValidation:
    ...
    def test_mission_delta_v_budgets(self):
        ...
        for mission, (min_dv, max_dv) in mission_types.items():
            ...
            assert (
                3000 <= min_dv_ms <= 8000
            ), f"{mission} minimum delta-v unrealistic: {min_dv_ms} m/s"
            assert (
                3000 <= max_dv_ms <= 8000
            ), f"{mission} maximum delta-v unrealistic: {max_dv_ms} m/s"
