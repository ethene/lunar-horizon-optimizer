...
class TestTrajectoryConstants:
    ...
    def test_relative_sizes(self):
        ...
        assert (
            PhysicalConstants.EARTH_MU > PhysicalConstants.MOON_MU * 80
        )  # Earth is ~81 times more massive
        # Earth should be larger than Moon
        assert (
            PhysicalConstants.EARTH_RADIUS > PhysicalConstants.MOON_RADIUS * 3
        )  # Earth is ~3.7 times larger
...
class TestManeuver:
    ...
    def test_maneuver_creation(self):
        ...
        maneuver = Maneuver(delta_v=delta_v, epoch=epoch, description=description)
    ...
    def test_maneuver_magnitude(self):
        ...
        Maneuver(delta_v=delta_v, epoch=epoch)

        # Calculate magnitude manually for comparison and verify maneuver was created
